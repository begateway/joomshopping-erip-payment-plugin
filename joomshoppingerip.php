...
class PlgSystemJoomShoppingErip extends JPlugin
{
	...
	public function onBeforeChangeOrderStatusAdmin($args){
		...
		$post_data["request"]["notification_url"] = JURI::root()."plugins/system/joomshoppingerip/libraries/callback.php";
		...
		if (isset($response_format->response->errors)) {
			...
			$mainframe->redirect('index.php?option=com_jshopping&controller=orders&task=show&order_id='.$order_details->order_id, $response_format->response->message, error);
			exit;
		}
		$config = JFactory::getConfig();
		try{
			$return = JFactory::getMailer()->sendMail($config->get('mailfrom'), $config->get('fromname'), $joomla_user_details->email, JText::_('PLG_JSERIPPAYMENT_EMAIL_INSTRUCTION_SUBJECT'), JText::sprintf('PLG_JSERIPPAYMENT_EMAIL_INSTRUCTION', $user_details->f_name . " " . $user_details->l_name, $order_details->order_id, JURI::root(), $payment_format['company_name'], $payment_format['tree_path_email']));		
		}
